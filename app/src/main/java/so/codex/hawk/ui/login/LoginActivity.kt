    ...
    private fun loginEventHandling(event: LoginEvent) {
        when (event) {
            ...
            LoginEvent.LOGIN_ERROR -> {
                notificationContainer.show(NotificationModel("Ошибка входа",NotificationType.ERROR))
            }
            LoginEvent.INTERNET_ERROR -> {
                notificationContainer.show(NotificationModel("Нет интернета",NotificationType.ERROR))
            }
        }
    }
}
